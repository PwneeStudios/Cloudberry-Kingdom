...
/*
	...
	float BodyIndicator = (_a + _b + _c + _d) / 4; //tex2D(SceneSampler, uv).r;
	float2 _uv = tex2D(SceneSampler, uv).gb;
	float offset = sin(16*_uv.x) * cos(16*_uv.y);
	_uv.x += .013 * cos(8*t + 5*offset);
	_uv.y += .013 * sin(8*t + 5*offset);
	
	float4 Color = tex2D(TextureSampler, _uv);

	Color.rgb += .025 * (1 + cos(7*t + 8*offset));
