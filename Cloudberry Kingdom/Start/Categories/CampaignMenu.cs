...
namespace CloudberryKingdom
{
    public class CampaignMenu : StartMenuBase
    {
        ...
        public CampaignMenu()
        {
...
#if PC_VERSION
            PressA = new EzText("Select difficulty", Tools.Font_DylanThin42);
#else
            PressA = new EzText("Press " + ButtonString.Go(100) + " to select", Tools.Font_DylanThin42);
#endif
            PressA.Scale *= .9f;
            PressA.Pos = new Vector2(-1686.508f, 805.5554f);
            MyPile.Add(PressA);

            PressY = new EzText("Press " + ButtonString.Y(100) + " for high scores", Tools.Font_DylanThin42);
            PressY.Scale *= .5f;
#if PC_VERSION
            ...
            name = new EzText(DifficultyNames[1].ToLower(), Tools.Font_DylanThin42, Centered, true);
            SetHeaderProperties(name);
            EasyColor(name);
            name.Pos = pos;
            Names.Add(name);

            name = new EzText(DifficultyNames[2].ToLower(), Tools.Font_DylanThin42, Centered, true);
            SetHeaderProperties(name);
            UnpleasantColor(name);
            name.Pos = pos;
            Names.Add(name);

            name = new EzText(DifficultyNames[3].ToLower(), Tools.Font_DylanThin42, Centered, true);
            SetHeaderProperties(name);
            AbusiveColor(name);
            name.Pos = pos;
            Names.Add(name);

            name = new EzText(DifficultyNames[4].ToLower(), Tools.Font_DylanThin42, Centered, true);
            SetHeaderProperties(name);
            HardcoreColor(name);
            name.Pos = pos;
            Names.Add(name);

            name = new EzText(DifficultyNames[5].ToLower(), Tools.Font_DylanThin42, Centered, true);
            SetHeaderProperties(name);
        ...
        void RemoveRain()
        {
            MyGame.MyGameObjects.ForEach(obj => { if (obj is Rain) obj.Release(); });
        }
        ...
        void SetCastleIndex(int Index)
        {
            ...
            // Rain for Masochistic
            if (MyGame != null)
            {
                if (CastleIndex >= 3)
                {
                    if (!MyGame.MyGameObjects.Any(obj => obj is Rain))
                        MyGame.AddGameObject(new Rain());
                }
                else
                    RemoveRain();
            }
        ...
        public void ShowHighScore()
        {
            ...
            ScoreText = new EzText(str, Tools.Font_DylanThin42, 1450, false, true, .6f);
