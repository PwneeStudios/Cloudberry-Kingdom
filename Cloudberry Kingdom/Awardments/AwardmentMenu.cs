...
namespace CloudberryKingdom.Awards
{
    public class AwardmentMenu : StartMenuBase
    {
        ...
        public AwardmentMenu()
        {
            ...
            // Header
            //EzText Header = new EzText("Awardments!", Tools.Font_Dylan60);
            //MyPile.Add(Header);
            //Header.Pos =
            ...
            // Header
            MenuItem Header = new MenuItem(new EzText("Awardments!", Tools.Font_Dylan60));
            MyMenu.Add(Header);
            Header.Pos =
            ...
            FreeplayText = new EzText("Freeplay:", Tools.Font_DylanThin42);
        ...
        void SetProgressText(Awardment ThisAward)
        {
            ...
            if (ThisAward == Awardments.JumpAlot)
            {
                if (MaxJumps <= Awardments.LotsOfJumps)
                    ProgressText = new EzText("Progress:{c26,188,241,255}" + MaxJumps + " / " + Awardments.LotsOfJumps, Tools.Font_DylanThin42);
                else
                    ProgressText = new EzText("Jumps:{c26,188,241,255}" + MaxJumps, Tools.Font_DylanThin42);
            }

            // Hero Rush
            if (ThisAward == Awardments.HeroRush_Score)
                ProgressText = new EzText("High Score:{c26,188,241,255}" + SaveGroup.HeroRushHighScore.Top, Tools.Font_DylanThin42);

            // Hero Rush 2
            if (ThisAward == Awardments.HeroRush2_Score)
                ProgressText = new EzText("High Score:{c26,188,241,255}" + SaveGroup.HeroRush2HighScore.Top, Tools.Font_DylanThin42);

            // Escalation
            if (ThisAward == Awardments.Escalation_Levels)
                ProgressText = new EzText("Highest Level:{c26,188,241,255}" + SaveGroup.EscalationHighLevel.Top, Tools.Font_DylanThin42);
            ...
            if (ThisAward == Awardments.FastCampaign2)
            {
                ...
                if (hsc.Scores.Count > 0 && hsc.Top > 0 && hsc.Top < 62 * 60 * 10000)
                    ProgressText = new EzText("Best Time:{c26,188,241,255}" + SaveGroup.CampaignScores[2].Time.Scores[0]._ToString(ScoreEntry.Format.Time), Tools.Font_DylanThin42);
            }
        ...
        void SetDescription(string text)
        {
            ...
            Description = new EzText(text, Tools.Font_DylanThin42, 885, false, false, .63f);
