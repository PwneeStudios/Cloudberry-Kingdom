...
namespace CloudberryKingdom
{
    ...
    public class PieceQuad
    {
            ...
            while (line != null)
            {
                ...
                if (line.StartsWith("["))
                {
                    ...
                    switch (type)
                    {
                        ...
                        case "LeftWidth": if (IsQM(line)) { Left.MyTexture.Load(); Data.LeftWidth = Left.MyTexture.Tex.Width; }  else Data.LeftWidth = float.Parse(line); break;
                        case "RightWidth": if (IsQM(line)) { Right.MyTexture.Load(); Data.RightWidth = Right.MyTexture.Tex.Width; } else Data.RightWidth = float.Parse(line); break;
                        case "TopWidth": if (IsQM(line)) { Top.MyTexture.Load(); Data.TopWidth = Top.MyTexture.Tex.Height; } else Data.TopWidth = float.Parse(line); break;
                        case "BottomWidth": if (IsQM(line)) { Bottom.MyTexture.Load(); Data.BottomWidth = Bottom.MyTexture.Tex.Height; } else Data.BottomWidth = float.Parse(line); break;
        ...
        void PreventWrap()
        {
            TR.U_Wrap = TR.V_Wrap = false;
            Top.U_Wrap = Top.V_Wrap = false;
            TL.U_Wrap = TL.V_Wrap = false;
            Right.U_Wrap = Right.V_Wrap = false;
            Center.U_Wrap = Center.V_Wrap = false;
            Left.U_Wrap = Left.V_Wrap = false;
            BR.U_Wrap = BR.V_Wrap = false;
            Bottom.U_Wrap = Bottom.V_Wrap = false;
            BL.U_Wrap = BL.V_Wrap = false;
        }
