...
namespace Drawing
{
    public class Game_Editor : Microsoft.Xna.Framework.Game
    {
        ...
        protected override void LoadContent()
        {
            ...
            Font1 = Content.Load<SpriteFont>("Fonts/MedFont");
        ...
        protected override void Update(GameTime gameTime)
        {
            ...
            if (!ShowTextureSelection)
            {
                ...
                if (KeybState.IsKeyDownCustom(Microsoft.Xna.Framework.Input.Keys.OemQuotes) && !PrevKeyboardState.IsKeyDownCustom(Microsoft.Xna.Framework.Input.Keys.OemQuotes))
                {
                    if (ChangeAll)
                        foreach (BaseQuad quad in MainObject.QuadList)
                            if (quad is Quad)
                                ((Quad)quad).ModifyAllRecords(anim, frame);
            ...
            if (!ShowTextureSelection && MouseOnScreen(CurrentMouse))
            {
                if (!CameraMove)
                {
                    ...
                    // Move selected quads
                    if (LeftMouseButtonState == Microsoft.Xna.Framework.Input.ButtonState.Pressed && SelectedPoints.Count == 0)
                        foreach (BaseQuad _quad in SelectedQuads)
                        {
                            Quad quad = _quad as Quad;
                            if (quad != null)
                                quad.Center.Move(quad.Center.Pos + MouseDelta);
                            BendableQuad bquad = _quad as BendableQuad;
                            if (bquad != null)
                                foreach (ObjectVector point in bquad.MySpline.GetObjectVectors())
        ...
        private void DrawScene()
        {
            ...
            */
            ...
            if (!Tools.ScreenshotMode)
            {
                ...
                if (ChangeAll)
                    spriteBatch.DrawString(Font1, "ModAll", new Vector2(5, 45), Color.Red);
                Vector2 pos = new Vector2(12, 50);
                foreach (BaseQuad quad in SelectedQuads)
                {
                    spriteBatch.DrawString(Font1, quad.Name, pos, Color.Blue);
                    pos.Y += 20;
                }
                foreach (ObjectBox box in SelectedBoxes)
                {
