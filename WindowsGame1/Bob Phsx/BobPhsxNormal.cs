...
namespace CloudberryKingdom
{
    public class BobPhsxNormal : BobPhsx
    {
        ...
        public override void PhsxStep2()
        {
            ...
            {
                Vector2 pos = new Vector2(-45f, -70);
                Vector2 dir = new Vector2(-.115f, -1.1f);
                if (Ducking)
                {
                    dir = new Vector2(-.35f, -.8f);
                    pos += new Vector2(-35, 20f);
                }
        ...
        public virtual void DoXAccel()
        {
            ...
            if (MyBob.CurInput.xVec.X < -.15f && xVel > -MaxSpeed)
            {
                if (xVel <= 0)
                    ...
                    if (!(Ducking && OnGround))
                        xVel += 2.7f * accel * MyBob.CurInput.xVec.X;

                if (xVel < 0 && xVel > -MaxSpeed / 10)
                    xVel = -MaxSpeed / 10;
            }
            if (MyBob.CurInput.xVec.X > .15f && xVel < MaxSpeed)
            {
        ...
        void GenerateInput_Right(int CurPhsxStep)
        {
            ...
            if (AutoMoveType == 1)// || blobStartJumpCount > 0)
