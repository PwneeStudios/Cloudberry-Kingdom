...
namespace Drawing
{
    ...
    {
        public static int ObjectClassVersionNumber = 53;
            ...
            if (BoxList.Count > 0)
            {
                ...
                if (AnimGroup == null)
                {
                    ...
                    if (ContainedQuadAngle != 0)
                    {
                        ...
                        effect.effect.CurrentTechnique = HoldTechnique;// effect.effect.Techniques["Simplest"];
                    }
                    else
                        Draw(Tools.EffectWad, false, ObjectDrawOrder.BeforeOutline);
                    Tools.QDrawer.Flush();
        ...
        public SpriteAnim AnimToSpriteFrames(int anim, int NumFrames, bool Loop, float StartT, float EndT, Vector2 Padding)
        {
            ...
            for (int i = 0; i < NumFrames; i++)
            {
                ...
                //Sprites.Frames[i].Save("Boob_" + anim.ToString() + "_" + i.ToString() + ".png", ImageFileFormat.Png);
            }
